print-%  : ; @echo $* = $($*)
PROJECT_NAME   = Nicole
COPYRIGHT      = "Nicole Contributors."
PROJECT_PATH   = nicole
SHELL          = /bin/bash
SOURCE_FOLDERS = nicole
PYTHON_FILES   = $(shell find $(SOURCE_FOLDERS) -type f -name "*.py" -o -name "*.pyi") nicole_cli_chat.py inference_nicole.py
...
format: py-format-install ruff-install addlicense-install
	...
	addlicense -c $(COPYRIGHT) -ignore tests/coverage.xml -l mit -y 2023-$(shell date +"%Y") $(SOURCE_FOLDERS) nicole_cli_chat.py inference_nicole.py
